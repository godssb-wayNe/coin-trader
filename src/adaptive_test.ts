...
/**
 * 🌊 [유동적 슬롯 배분(Adaptive Slot Sizing) 10년치 백테스터]
 */
export class AdaptiveSlotComparator {
  ...
  public async run(): Promise<void> {
    ...
    // 2. 🌟 사용자 제안: 최대 5종목 내에서 가용현금/빈슬롯 실시간 유동적 분할
    const adaptiveResult = this.simulate(symbols, marketCandlesMap, btcCandles, true);

    console.log('\n======================================================');
    console.log(`🌊 [사용자 제안: 5종목 고정 슬롯 vs 5종목 유동적 슬롯 배분 비교]`);
    console.log('======================================================');
    console.log(`구분                     5종목 고정 20% 분할         5종목 유동적 슬롯 배분 (Adaptive)`);
    console.log('------------------------------------------------------');
    console.log(`• 시작 원금              100,000원                   100,000원`);
    console.log(`• 최종 평가자산          ${Math.round(fixedResult.finalCapital).toLocaleString()}원                   ${Math.round(adaptiveResult.finalCapital).toLocaleString()}원 🌟`);
    console.log(`• 누적 순손익            +${Math.round(fixedResult.netProfit).toLocaleString()}원 (+${fixedResult.returnPercent.toFixed(2)}%)        +${Math.round(adaptiveResult.netProfit).toLocaleString()}원 (+${adaptiveResult.returnPercent.toFixed(2)}%) 🚀`);
    console.log(`• 최대 낙폭(MDD)         -${fixedResult.maxDrawdown.toFixed(2)}%                    -${adaptiveResult.maxDrawdown.toFixed(2)}%`);
    console.log('======================================================\n');
  }

  private simulate(symbols: string[], map: Map<string, Candle[]>, btcCandles: Candle[], isAdaptive: boolean) {
    let cash = 100000;
    let peak = 100000;
    let maxDrawdown = 0;
    ...
    for (let i = 200; i < btcCandles.length; i++) {
      ...
      for (const s of symbols) {
        ...
        // 🌊 유동적 매수 금액 산출 (남은 빈 슬롯 수와 남은 가용 현금을 나눗셈)
        const openSlots = maxPositions - positions.length;
        let investAmount = 20000;

        if (isAdaptive) {
          // 남은 빈 슬롯당 가용 현금을 알뜰하게 100% 분배 (최소 5천원 ~ 가용현금의 95% / openSlots)
          investAmount = Math.max(5000, Math.floor((cash * 0.95) / openSlots));
        } else {
          // 기존 고정 1/5
          investAmount = Math.min(Math.floor(equity / maxPositions), Math.floor(cash * 0.95));
        }

        if (cash < investAmount || investAmount < 5000) break;
...
async function main() {
  const runner = new AdaptiveSlotComparator();
  await runner.run();
}
